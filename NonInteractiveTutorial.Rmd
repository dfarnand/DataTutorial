---
title: "Non-Interactive Tutorial"
...
Part of the data cleaning process involves recoding data into a more useful form. One of the simple things we can do is make variables with only two options into the form of either True or False. We can easily apply this conversion to multiple variables using Dplyr's `mutate_each` function. The second argument, `funs(. == "1-Yes")`, takes each variable and evaluates it as equal to the value "1-Yes", making it into a T/F logical variable.

```{r tf-recode, warning=FALSE}
...
The following histograms are given to represent what can be seen in the interactive one:

```{r, echo=F}
barplot(table(hs_merged$Urban.centric.Locale.sch),  xlab="Urbanicity")
barplot(table(hs_merged$Urban.Cat.sch), xlab="Urbanicity (collapsed)")

hist(hs_merged$Asian.or.Asian.Pacific.Islander.Students.sch/hs_merged$Total.Students.sch,
     xlab = "Percent Asian")

hist(hs_merged$Hispanic.Students.sch/hs_merged$Total.Students.sch,
     xlab = "Percent Hispanic")

hist(hs_merged$Black.Students.sch/hs_merged$Total.Students.sch,
     xlab = "Percent Black")

hist(hs_merged$White.Students.sch/hs_merged$Total.Students.sch,
     xlab = "Percent White")
```
